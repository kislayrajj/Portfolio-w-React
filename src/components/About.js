...
const themeColor=useSelector((state)=>state.theme.themeColor);
  ...
  return (
    <div className="h-screen overflow-hidden" ref={aboutRef}>
      <div className={`about-top h-1/2  relative pt-8 ${themeColor === "dark" ? "bg-black" :"bg-gradient-to-r from-gray-50 via-gray-200 to-gray-50"}`}>
        <div className={`w-full text-start text-3xl pl-12 theme-font font-semibold  ${themeColor === "dark" ? "text-green-400" : "text-blue-600"}`}>
          About Me
        </div>
        ...
        <motion.div
          ...
          <i className={`fa-regular  theme-font font-bold uppercase ${themeColor==="dark" ? "text-white":"text-green-500"}`}>
            {aboutInfo?.name}
          </i>
        </motion.div>
      </div>
      <div className="about-bottom h-1/2 bg-gradient-to-r from-slate-900 to-slate-900">
        <div className="w-full flex flex-col lg:flex-row justify-between gap-10 items-center p-2 pt-12">
          ...
          <div className={`follow text-2xl lg:text-4xl flex gap-10 ${themeColor ==="dark" ? "":"text-white"}`}>
            <motion.span
