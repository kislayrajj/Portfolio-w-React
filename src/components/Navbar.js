...
const Navbar = () => {
  ...
  const downloadResume = () => {
    ...
    const takeDownloadConfirmation = window.confirm(
      "This will download my Resume file! Continue?"
    );
    if (takeDownloadConfirmation) {
      fetch(url)
        .then((response) => {
          if (!response.ok) {
        ...
        .then((blob) => {
          ...
          saveAs(blob, "kislayRaj_resume_webDeveloper _1.5yr_exp.pdf");
        })
  ...
  return (
    <div ref={menuRef} className="flex justify-center relative">
      <nav
        className={`navbar  h-auto w-11/12 m-2 rounded-3xl flex justify-around p-3 fixed  z-20 ${
          themeColor === "dark" ? "" : "bg-white/80"
        }`}>
        {/* <img
          src={Logo}
        ...
        <div
          ...
          {isBurger ? (
            <i className="fa-solid fa-xmark h-10 w-10"></i>
          ) : (
            <i className="fa-solid fa-ellipsis-vertical h-10 w-10"></i>
          )}
        </div>
        <div className={`menu-1 m-2 `}>
          <ul
            className={`md:flex lg:text-base xl:text-xl ${
              themeColor === "dark" ? "text-white" : "text-black"
            }  lg:gap-20 md:gap-10  absolute lg:static  w-screen md:z-auto left-0 md:w-auto md:pl-0 pl-20 transition-all duration-500 ease-in-out ${
              isBurger ? "left-0 top-24 " : "left-[-800px] top-20"
            } ${
              themeColor === "dark"
                ? "bg-gray-900/75 lg:bg-inherit p-2 lg:p-0 xl:pl-20"
                : " bg-white/75 md:bg-inherit"
            }`}>
            <Link smooth to="/#main-content">
              <motion.li
                initial={{ opacity: 0, y: -200 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 1 }}
                className={`cursor-pointer hover:text-green-500  hover:font-bold transition-all ease-in-out duration-100 ${
                  isActive === "home"
                    ? "xl:text-2xl lg:text-lg text-green-500 border-b-2 border-green-500 "
                    : ""
                }`}
            ...
            <Link smooth to="/projects">
              <motion.li
                ...
                <button
                  className={`border-2 border-green-600 rounded-sm pl-2 pr-2 transition-all ease-in-out duration-100 ${
                    isActive === "projects"
                      ? "xl:text-2xl lg:text-lg text-green-500 border-b-2 border-green-500"
                      : ""
                  } button-glitch`}>
        ...
        <a
          ...
          <motion.button
            ...
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 1, delay: 1 }}
            className={`border-2 border-l-black border-t-black border-r-teal-200 border-b-teal-100 bg-transparent backdrop-blur-[1px] p-1 pl-5 pr-5  absolute md:static top-14 right-6  md:mr-0 md:mt-1  hover:border-r-black hover:border-b-black hover:border-l-teal-200 hover:border-t-teal-100 transition-all duration-200 ${
              themeColor === "dark"
                ? "hover:bg-black/50 hover:text-green-300"
                : "text-pink-500 hover:bg-white/50 hover:text-green-600"
            }`}>
            Hire
          </motion.button>
        </a>
        <div className="older_smaller_screen_theme_icon lg:center lg:mt-2 text-sm  2xl:text-xl absolute right-10 top-6 md:right-44 lg:right-44  lg:static">
          <motion.div
            ...
            <i
              className={`fa-solid fa-palette hover:scale-105 ${
                !isThemeMenuHidden ? "fa-fade" : ""
              }`}></i>
          </motion.div>
          <div className="relative older_smaller_screen_theme_menu_parent">
            {!isThemeMenuHidden && (
              <motion.div
                ...
                className={`older_smaller_screen_theme_menu absolute -left-3 xl:left-0 rounded p-1 lg:p-2 ${
                  themeColor === "dark"
                ...
                {colorOptions.map((color) => (
                  <p
                    ...
                    className={`older_smaller_screen_theme_menu_list cursor-pointer  px-2 py-1 mt-1 rounded-md transition-all ease-in-out duration-300 center ${
                      themeColor === color.value ? "bg-green-400" : ""
                    }`}>
                    {color.name}
                  </p>
                ))}
